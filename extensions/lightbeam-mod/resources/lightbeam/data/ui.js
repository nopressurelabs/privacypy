...
function parseConnections(connections) {
  allJson = '{"name":"hyperme","children":[';
  connections.forEach(function (connection) {
    allJson += '{"node":"' + connection[0] + '","children":[';
    connection.forEach(function (connField) {
      allJson += '{"selector":"' + connField + '"},'
    });
    allJson += ']},';
  });
  allJson += ']}';
  downloadAsJson([JSON.stringify(allJson)], 'lightbeamData.json');
}

document.querySelector(".download").addEventListener('click', function (evt) {
  // console.log('received export data');
  parseConnections(allConnections);
  evt.preventDefault();
